...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        select_keys.extend(["old_log_probs", "ref_log_probs", "teacher_log_probs", "advantages"]) # teacher policy, by bk
        non_tensor_select_keys = ["multi_modal_inputs"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    # Teacher and Student Logit KLD, by bk
                    if "teacher_log_probs" in model_inputs:
                        teacher_log_probs = model_inputs["teacher_log_probs"]
                        # compute kl loss
                        teacher_kld = compute_kl(
                            log_probs=log_probs,
                            ref_log_probs=teacher_log_probs,
                            kl_penalty='jsd',
                        )
                        # mask out padding tokens and sum to per-sample scalar
                        teacher_kld = (teacher_kld * response_mask).sum(dim=-1)     # [B, L] -> [B]
                        response_len = response_mask.sum(dim=-1).clamp(min=1)   # [B]
                        mean_jsd = (teacher_kld / response_len).mean()               # per-token JSD, bounded [0, log(2)]
                        
                        teacher_kl_coef = 0.1
                        pg_loss = pg_loss + teacher_kl_coef * mean_jsd
                        metrics["actor/teacher_kl_loss"] = mean_jsd.detach().item()
