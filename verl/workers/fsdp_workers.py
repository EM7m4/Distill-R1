...
class FSDPWorker(Worker):
    def __init__(
        self,
        config: WorkerConfig,
        role: Literal["actor", "critic", "rollout", "ref", "teacher_rollout", "actor_rollout", "actor_rollout_ref"],
    ):
        super().__init__()
        self.config = config
        self.role = role
        ...
        self._has_rollout = self.role in ["rollout", "actor_rollout", "actor_rollout_ref", "teacher_rollout"]
        self._has_teacher = self.role == "teacher_rollout" # teacher policy, by bk
    ...
    def _build_model_optimizer(
        ...
        padding_free: bool,
        role: Literal["actor", "critic", "ref", "teacher_rollout"],
    ) -> None:
        ...
        if role == "ref" or role == "teacher_rollout": # teacher policy, by bk
    ...
    def _build_rollout(self, model_path, enable_sleep_mode: bool) -> None:
    ...
    def init_model(self):
        ...
        if self._has_teacher:
            self._build_model_optimizer(
                ...
                role="teacher_rollout",
            )
            from .teacher.dp_teacher import DataParallelPPOTeacher

            self.teacher_policy = DataParallelPPOTeacher(
        ...
        if self._has_rollout: # must after actor, by bk
            if self.role == "actor_rollout_ref":
                self._build_rollout(self.config.actor.model.model_path, enable_sleep_mode=True)
            elif self.role == "teacher_rollout":
                self._build_rollout(self.config.teacher.model.model_path, enable_sleep_mode=True)
    ...
    def compute_teacher_log_probs(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(
                tensors={"teacher_log_probs": output}, meta_info={"temperature": self.config.rollout.temperature}
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
