...
class Role(IntEnum):
    ...
    TeacherRollout = auto() # teacher actor, by bk
    Critic = auto()
    RewardModel = auto()
...
class RayPPOTrainer:
    ...
    def init_workers(self) -> None:
        ...
        # create teacher and rollout, by bk
        if self.use_teacher:
            if self.hybrid_engine:
                resource_pool = self.resource_pool_manager.get_resource_pool(Role.TeacherRollout)
                teacher_cls = RayClassWithInitArgs(
                    cls=self.role_worker_mapping[Role.TeacherRollout], config=self.config.worker, role="teacher_rollout"
                )
                self.resource_pool_to_cls[resource_pool]["teacher_rollout"] = teacher_cls
            else:
                raise NotImplementedError
        ...
        # teacher, by bk
        if self.use_teacher:
            self.teacher_rollout_wg = all_wg["teacher_rollout"]
    ...
    def _load_checkpoint(self) -> None:
        ...
        # 경로 자체가 없으면 바로 return
        if not os.path.exists(save_checkpoint_path):
            return

        has_global_step_dir = any(
            name.startswith("global_step")
            and os.path.isdir(os.path.join(save_checkpoint_path, name))
            for name in os.listdir(save_checkpoint_path)
        )

        # global_step_xxx 폴더가 없으면 바로 return
        if not has_global_step_dir:
            return
        ...
        for name in os.listdir(save_checkpoint_path):
            ...
            if match:
                ...
                if step > max_step:
                    max_step = step
                    max_step_name = name
        load_checkpoint_path = os.path.join(save_checkpoint_path, max_step_name)
        ###################### LBK CODE ######################

        # Easy R1 Original Code
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        batch = None
        teacher_batches = [] if self.use_teacher else None
        ...
        while True:
            ...
            if teacher_batches is not None:
                teacher_batches.append(gen_batch)
            ...
            if current_batch_size < rollout_batch_size:
                ...
                if teacher_batches is not None:
                    self._teacher_batches = DataProto.concat(teacher_batches)[:rollout_batch_size]
    ...
    def fit(self):
        ...
        # teacher uses separate GPUs and is frozen, so load vLLM once and keep it alive
        # if self.use_teacher:
        #     self.teacher_rollout_wg.prepare_rollout_engine()
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # teacher rollout: generate teacher responses on same prompts, by bk
                if self.use_teacher:
                    with timer("teacher_gen", timing_raw):
                        self.teacher_rollout_wg.prepare_rollout_engine()
                        teacher_gen = self.teacher_rollout_wg.generate_sequences(self._teacher_batches)
                        self.teacher_rollout_wg.release_rollout_engine()

                        # copy uid and ground_truth so GRPO groups student+teacher together
                        teacher_gen.non_tensor_batch["uid"] = batch.non_tensor_batch["uid"]
                        teacher_gen.non_tensor_batch["ground_truth"] = batch.non_tensor_batch["ground_truth"]

                        # concat along batch dim: [128+128] = [256], each prompt has 8 responses
                        batch = DataProto.concat([batch, teacher_gen])
                ...
                # print
                # self.processor.batch_decode(batch.batch['input_ids'])[0].replace('<|endoftext|>', '').replace('<|image_pad|>', '')
                # self.processor.batch_decode(self._teacher_batches.batch['input_ids'])[0].replace('<|endoftext|>', '').replace('<|image_pad|>', '')
                # list(filter(lambda x: not x.startswith('__'), dir(self._teacher_batches)))
                ...
                # compute teacher log_probs (for distillation), by bk
                if self.use_teacher:
                    with timer("teacher", timing_raw):
                        teacher_log_probs = self.teacher_rollout_wg.compute_teacher_log_probs(batch)
