...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        role_worker_mapping = {
            ...
            Role.TeacherRollout: ray.remote(FSDPWorker), # teacher policy, by bk
        }
        student_pool_id = "student_pool"
        teacher_pool_id = "teacher_pool"
        n_gpus = config.trainer.n_gpus_per_node
        assert n_gpus % 2 == 0, "GPU should be divisible by 2"
        half = n_gpus // 2

        resource_pool_spec = {
            student_pool_id: [half] * config.trainer.nnodes,      # actor/student/ref/critic
            teacher_pool_id: [half] * config.trainer.nnodes,     # teacher rollout
        }
        mapping = {
            Role.ActorRolloutRef: student_pool_id,
            Role.TeacherRollout: teacher_pool_id,
        }
        resource_pool_manager = ResourcePoolManager(resource_pool_spec=resource_pool_spec, mapping=mapping)

        if config.trainer.debug: breakpoint() # Debugger breakpoint
